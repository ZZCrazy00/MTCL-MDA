...
class NCL(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.ssl_temp = config['ssl_temp']  # InfoNCE的温度超参数
    ...
    def predict(self, interaction):
        ...
        user = user.sort(0, False).values
        item = item.sort(0, False).values
        user_all_embeddings, item_all_embeddings, embeddings_list = self.forward()
        ...
        for i in range(len(u_embeddings)):
            if u_embeddings[i] not in dis:
                dis.append(u_embeddings[i])
